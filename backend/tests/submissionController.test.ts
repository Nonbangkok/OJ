...
jest.mock('../middleware/auth', () => ({
    requireAuth: (req: Request, res: Response, next: NextFunction) => {
        if (req.session) {
            // Keep an explicitly staged role (staff/admin apps below) instead
            // of stomping it — the default stays a regular user.
...
describe('Submission Controller', () => {
    ...
    describe('GET /submissions', () => {
        ...
        it('should return 200 for staff on a contest feed without a participant row', async () => {
            ...
            staffApp.use((req: Request, res: Response, next: NextFunction) => {
                if (req.session) {
                    req.session.userId = 1;
