...
router.post('/submit', auth_1.requireAuth, upload_1.memoryUpload.none(), async (req, res) => {
    ...
    try {
        ...
        if (contestId) {
            ...
            const contestRes = await db.query('SELECT id, status, start_time, end_time FROM contests WHERE id = $1', [contestId]);
            ...
            const problemRes = await db.query('SELECT 1 FROM problems WHERE id = $1 AND contest_id = $2', [problemId, contestId]);
            ...
            const submissionRes = await db.query(`INSERT INTO contest_submissions (contest_id, user_id, problem_id, code, language, overall_status, score)
         VALUES ($1, $2, $3, $4, $5, $6, 0) RETURNING id`, [contestId, userId, problemId, code, language, constants_1.SUBMISSION_STATUS.PENDING]);
            ...
            (0, submissionService_1.processContestSubmission)(submissionId);
        }
        else {
            // Regular submission to main system
            // Check if problem is available (not in any contest and visible)
            const problemRes = await db.query('SELECT 1 FROM problems WHERE id = $1 AND is_visible = true AND contest_id IS NULL', [problemId]);
            if (problemRes.rows.length === 0) {
                return res.status(400).json({
                    message: 'Problem is not available for submission.'
                });
            }
            const submissionRes = await db.query(`INSERT INTO submissions (user_id, problem_id, code, language, overall_status, score)
         VALUES ($1, $2, $3, $4, $5, 0) RETURNING id`, [userId, problemId, code, language, constants_1.SUBMISSION_STATUS.PENDING]);
            const submissionId = submissionRes.rows[0].id;
            res.status(202).json({
            ...
            (0, submissionService_1.processSubmission)(submissionId);
        }
    }
    catch (dbError) {
...
router.get('/submissions', auth_1.requireAuth, async (req, res) => {
    ...
    const isStaffOrAdmin = role === constants_1.USER_ROLES.ADMIN || role === constants_1.USER_ROLES.STAFF;
    try {
...
router.get('/search/problems', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (contestId) {
            ...
            if (status === constants_1.CONTEST_STATUS.FINISHED) {
                result = await db.query(`SELECT problem_id as id, title FROM contest_problems 
           WHERE contest_id = $1 AND (problem_id ILIKE $2 OR title ILIKE $2) 
           LIMIT 10`, [contestId, `%${q}%`]);
            }
            else {
                result = await db.query(`SELECT id, title FROM problems 
           ...
           LIMIT 10`, [contestId, `%${q}%`]);
            ...
            // Global search
            result = await db.query('SELECT id, title FROM problems WHERE id ILIKE $1 OR title ILIKE $1 LIMIT 10', [`%${q}%`]);
        }
        res.json(result.rows);
    }
    catch (error) {
...
router.get('/search/users', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        let result;
        if (contestId) {
            result = await db.query(`SELECT u.username FROM users u
         JOIN contest_participants cp ON u.id = cp.user_id
         WHERE cp.contest_id = $1 AND u.username ILIKE $2
         LIMIT 10`, [contestId, `%${q}%`]);
        }
        else {
            result = await db.query('SELECT username FROM users WHERE username ILIKE $1 LIMIT 10', [`%${q}%`]);
        }
        res.json(result.rows);
    }
    catch (error) {
...
router.get('/submissions/:id', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        if (contestId) {
            ...
            result = await db.query(`SELECT cs.*, u.username 
         FROM contest_submissions cs
         LEFT JOIN users u ON cs.user_id = u.id
         WHERE cs.id = $1 AND cs.contest_id = $2`, [id, contestId]);
        }
        else {
            // Fetch regular submission
            result = await db.query(`SELECT s.*, u.username 
         FROM submissions s
...
router.get('/scoreboard', auth_1.requireAuth, async (req, res) => {
    try {
