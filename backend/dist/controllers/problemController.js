...
const batchUploadService_1 = require("../services/batchUploadService");
const constants_1 = require("../constants");
const router = express_1.default.Router();
const progressMap = new Map();
router.get('/problems-with-stats', auth_1.requireAuth, async (req, res) => {
...
router.get('/problems', async (req, res) => {
    try {
        const result = await db.query('SELECT id, title, author FROM problems WHERE is_visible = true AND contest_id IS NULL ORDER BY id');
        res.json(result.rows);
    }
    catch (error) {
...
router.get('/problems/:id', async (req, res) => {
    ...
    try {
        ...
        const isStaffOrAdmin = req.session.role === constants_1.USER_ROLES.ADMIN || req.session.role === constants_1.USER_ROLES.STAFF;
...
router.get('/admin/problems/:id', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    const { id } = req.params;
    try {
...
router.get('/problems/:id/pdf', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    try {
...
router.post('/admin/problems', auth_1.requireAuth, auth_1.requireStaffOrAdmin, [
    (0, express_validator_1.body)('id').isLength({ min: 1 }).trim().escape(),
    (0, express_validator_1.body)('title').isLength({ min: constants_1.PROBLEM_VALIDATION.MIN_TITLE_LENGTH }).trim(),
    (0, express_validator_1.body)('author').isLength({ min: constants_1.PROBLEM_VALIDATION.MIN_AUTHOR_LENGTH }).trim(),
    (0, express_validator_1.body)('time_limit_ms').isInt({ min: constants_1.PROBLEM_VALIDATION.MIN_TIME_LIMIT_MS }),
    (0, express_validator_1.body)('memory_limit_mb').isInt({ min: constants_1.PROBLEM_VALIDATION.MIN_MEMORY_LIMIT_MB })
], async (req, res) => {
    ...
    try {
        const result = await db.query('INSERT INTO problems (id, title, author, time_limit_ms, memory_limit_mb) VALUES ($1, $2, $3, $4, $5) RETURNING *', [id, title, author, time_limit_ms, memory_limit_mb]);
...
router.put('/admin/problems/:id', auth_1.requireAuth, auth_1.requireStaffOrAdmin, [
    (0, express_validator_1.body)('id').isLength({ min: 1 }).trim().escape(), // New ID
    (0, express_validator_1.body)('title').optional({ checkFalsy: true }).isLength({ min: constants_1.PROBLEM_VALIDATION.MIN_TITLE_LENGTH }).trim(),
    (0, express_validator_1.body)('author').optional({ checkFalsy: true }).isLength({ min: constants_1.PROBLEM_VALIDATION.MIN_AUTHOR_LENGTH }).trim(),
    (0, express_validator_1.body)('time_limit_ms').optional().isInt({ min: constants_1.PROBLEM_VALIDATION.MIN_TIME_LIMIT_MS }),
    (0, express_validator_1.body)('memory_limit_mb').optional().isInt({ min: constants_1.PROBLEM_VALIDATION.MIN_MEMORY_LIMIT_MB })
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        ...
        const result = await db.query('UPDATE problems SET id = $1, title = $2, author = $3, time_limit_ms = $4, memory_limit_mb = $5 WHERE id = $6 RETURNING *', [newId, title, author, time_limit_ms, memory_limit_mb, oldId]);
...
router.delete('/admin/problems/:id', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    const { id } = req.params;
    try {
...
router.get('/admin/problems', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    try {
...
router.put('/admin/problems/:id/visibility', auth_1.requireAuth, auth_1.requireStaffOrAdmin, [
    (0, express_validator_1.body)('isVisible').isBoolean()
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        const result = await db.query('UPDATE problems SET is_visible = $1 WHERE id = $2 RETURNING id, title, is_visible', [isVisible, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Problem not found' });
        }
        res.json({
...
router.post('/admin/problems/batch-upload', auth_1.requireAuth, auth_1.requireStaffOrAdmin, upload_1.diskUpload.single('problemsZip'), async (req, res) => {
    if (!req.file) {
    ...
    try {
        ...
        const batchResults = await (0, batchUploadService_1.processBatchUpload)(req.file.path, (progressData) => {
...
router.get('/admin/problems/batch-upload-progress/:progressId', auth_1.requireAuth, auth_1.requireStaffOrAdmin, (req, res) => {
...
router.post('/admin/problems/:id/upload', auth_1.requireAuth, auth_1.requireStaffOrAdmin, upload_1.memoryUpload.fields([
    ...
    try {
        ...
        if (testcasesZipFile) {
            ...
            await db.query('DELETE FROM testcases WHERE problem_id = $1', [id]);
            const zip = await unzipper_1.default.Open.buffer(testcasesZipFile.buffer);
            const testcaseFiles = {};
            ...
            for (const key of pairedCases) {
                ...
                await db.query('INSERT INTO testcases (problem_id, case_number, input_data, output_data) VALUES ($1, $2, $3, $4)', [id, caseCounter, inputData.toString('utf-8'), outputData.toString('utf-8')]);
                caseCounter++;
            }
        }
...
router.post('/admin/problems/export', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    ...
    try {
        for (const problemId of problemIds) {
            const problemRes = await db.query('SELECT id, title, author, time_limit_ms, memory_limit_mb, problem_pdf FROM problems WHERE id = $1', [problemId]);
