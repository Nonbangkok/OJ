...
const problemMigration = __importStar(require("../services/problemMigration"));
...
router.get('/contests', async (req, res) => {
    ...
    try {
        ...
        SELECT 
          ...
          CASE 
            WHEN c.status = '${constants_1.CONTEST_STATUS.FINISHED}' THEN COALESCE(finished_problems.problem_count, 0)
        ...
        LEFT JOIN (
          ...
          GROUP BY contest_id
        ) finished_problems ON c.id = finished_problems.contest_id AND c.status = '${constants_1.CONTEST_STATUS.FINISHED}'
        LEFT JOIN (
          SELECT contest_id, COUNT(*) as problem_count
          FROM problems
          WHERE contest_id IS NOT NULL
          GROUP BY contest_id
        ) active_problems ON c.id = active_problems.contest_id AND c.status != '${constants_1.CONTEST_STATUS.FINISHED}'
...
router.get('/admin/contests', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    try {
...
router.get('/admin/contests/available-problems', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    try {
...
router.get('/contests/:id', async (req, res) => {
    ...
    `, [id]);
      ...
      `, [id, userId]);
        ...
        let problems = [];
        if (contest.status === constants_1.CONTEST_STATUS.RUNNING || contest.status === constants_1.CONTEST_STATUS.FINISHED) {
            if (contest.status === constants_1.CONTEST_STATUS.FINISHED) {
...
// Join a contest
router.post('/contests/:id/join', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    const { userId } = req.session;
    try {
        ...
        try {
            await db.query('INSERT INTO contest_participants (contest_id, user_id) VALUES ($1, $2)', [id, userId]);
...
router.get('/contests/:id/scoreboard', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    try {
        ...
        else if (contest.status === constants_1.CONTEST_STATUS.RUNNING || contest.status === constants_1.CONTEST_STATUS.FINISHING) {
...
router.post('/admin/contests', auth_1.requireAuth, auth_1.requireStaffOrAdmin, [
    (0, express_validator_1.body)('title').isLength({ min: 1 }).trim(),
    (0, express_validator_1.body)('description').optional().trim(),
    (0, express_validator_1.body)('startTime').isISO8601().toDate(),
    (0, express_validator_1.body)('endTime').isISO8601().toDate()
], async (req, res) => {
...
router.put('/admin/contests/:id', auth_1.requireAuth, auth_1.requireStaffOrAdmin, [
    (0, express_validator_1.body)('title').isLength({ min: 1 }).trim(),
    (0, express_validator_1.body)('description').optional().trim(),
    (0, express_validator_1.body)('startTime').isISO8601().toDate(),
    (0, express_validator_1.body)('endTime').isISO8601().toDate()
...
router.delete('/admin/contests/:id', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        ...
        const contest = contestResult.rows[0];
        if (contest.status === constants_1.CONTEST_STATUS.RUNNING) {
            return res.status(400).json({
                message: 'Cannot delete a running contest'
            });
        }
        // Move problems back to main system before deleting
        await db.query('UPDATE problems SET contest_id = NULL WHERE contest_id = $1', [id]);
...
router.get('/admin/contests/:id/admin-problems', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const problems = await problemMigration.getProblemsInContest(parseInt(id));
        res.json(problems);
    }
    catch (error) {
...
router.post('/admin/contests/:id/problems', auth_1.requireAuth, auth_1.requireStaffOrAdmin, [
    (0, express_validator_1.body)('problemIds').isArray({ min: 1 }).withMessage('Must provide at least one problem ID'),
    (0, express_validator_1.body)('problemIds.*').isString().trim().escape(),
    (0, express_validator_1.body)('action').isIn(['move_to_contest', 'move_to_main']).withMessage('Action must be move_to_contest or move_to_main')
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
...
router.delete('/admin/contests/:id/problems/:problemId', auth_1.requireAuth, auth_1.requireStaffOrAdmin, async (req, res) => {
    ...
    try {
        ...
        const contest = contestResult.rows[0];
        if (contest.status !== 'scheduled' && contest.status !== constants_1.CONTEST_STATUS.RUNNING) {
            return res.status(400).json({
                message: 'Can only move problems from scheduled or running contests'
            });
        }
        // Move single problem back to main system
        const updateResult = await db.query('UPDATE problems SET contest_id = NULL WHERE id = $1 AND contest_id = $2 RETURNING id, title', [problemId, id]);
...
router.get('/contests/:id/problems', auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    const { userId } = req.session;
    try {
        // Check if contest exists
        ...
        const participantResult = await db.query('SELECT 1 FROM contest_participants WHERE contest_id = $1 AND user_id = $2', [id, userId]);
        ...
        if (![constants_1.CONTEST_STATUS.RUNNING, constants_1.CONTEST_STATUS.FINISHING, constants_1.CONTEST_STATUS.FINISHED].includes(contest.status)) {
...
router.get('/contests/:id/problems/:problemId', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (![constants_1.CONTEST_STATUS.RUNNING, constants_1.CONTEST_STATUS.FINISHING, constants_1.CONTEST_STATUS.FINISHED].includes(contestStatus)) {
            return res.status(403).json({ message: 'Contest is not active.' });
        }
        const participantRes = await db.query('SELECT 1 FROM contest_participants WHERE contest_id = $1 AND user_id = $2', [contestId, userId]);
        ...
        let problemRes;
        if (contestStatus === constants_1.CONTEST_STATUS.FINISHED) {
            // For finished contests, get data from the snapshot
            problemRes = await db.query('SELECT problem_id as id, title, author, time_limit_ms, memory_limit_mb, (problem_pdf IS NOT NULL) as has_pdf FROM contest_problems WHERE contest_id = $1 AND problem_id = $2', [contestId, problemId]);
        }
        else {
            // For running contests, get data from the main problems table
            problemRes = await db.query('SELECT id, title, author, time_limit_ms, memory_limit_mb, (problem_pdf IS NOT NULL) as has_pdf FROM problems WHERE id = $1 AND contest_id = $2', [problemId, contestId]);
        }
...
router.get('/contests/:id/problems/:problemId/pdf', auth_1.requireAuth, async (req, res) => {
    const { id: contestId, problemId } = req.params;
    const { userId } = req.session;
    try {
        ...
        let pdfRes;
        if (contestStatus === constants_1.CONTEST_STATUS.FINISHED) {
            pdfRes = await db.query('SELECT problem_pdf FROM contest_problems WHERE contest_id = $1 AND problem_id = $2', [contestId, problemId]);
        }
        else {
            pdfRes = await db.query('SELECT problem_pdf FROM problems WHERE id = $1 AND contest_id = $2', [problemId, contestId]);
        }
