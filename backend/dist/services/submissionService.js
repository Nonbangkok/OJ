...
const { judge } = require('./judgeService');
async function processSubmission(submissionId) {
    ...
    try {
        ...
        try {
            ...
            await db.query(`UPDATE submissions SET overall_status = 'Compilation Error', results = $1 WHERE id = $2`, [JSON.stringify([{ status: 'Compilation Error', output: compileError.stderr }]), submissionId]);
            return;
        }
        finally {
            fs.unlink(filePath, (err) => { if (err)
                console.error(`Error deleting .cpp file for sub ${submissionId}:`, err); });
        }
        ...
        await db.query(`UPDATE submissions
       SET overall_status = $1, score = $2, results = $3, max_time_ms = $4, max_memory_kb = $5
       WHERE id = $6`, [overallStatus, score, JSON.stringify(results), maxTimeMs, maxMemoryKb, submissionId]);
    }
    catch (error) {
        console.error(`Critical error processing submission ${submissionId}:`, error);
        try {
            await db.query(`UPDATE submissions SET overall_status = 'System Error' WHERE id = $1`, [submissionId]);
        }
        catch (dbError) {
        ...
        fs.unlink(outputPath, (err) => { if (err)
            console.error(`Error deleting .out file for sub ${submissionId}:`, err); });
    }
}
...
async function processContestSubmission(submissionId) {
    ...
    try {
        ...
        try {
            ...
            await db.query(`UPDATE contest_submissions SET overall_status = 'Compilation Error', results = $1 WHERE id = $2`, [JSON.stringify([{ status: 'Compilation Error', output: compileError.stderr }]), submissionId]);
            return;
        }
        finally {
            fs.unlink(filePath, (err) => { if (err)
                console.error(`Error deleting .cpp file for contest sub ${submissionId}:`, err); });
        }
        await db.query(`UPDATE contest_submissions SET overall_status = 'Running' WHERE id = $1`, [submissionId]);
        await fs.promises.chmod(outputPath, 0o755);
        const judgeResult = await judge(problem_id, outputPath);
        const { results, score, overallStatus, maxTimeMs, maxMemoryKb } = judgeResult;
        await db.query(`UPDATE contest_submissions
       SET overall_status = $1, score = $2, results = $3, max_time_ms = $4, max_memory_kb = $5
       WHERE id = $6`, [overallStatus, score, JSON.stringify(results), maxTimeMs, maxMemoryKb, submissionId]);
    }
    catch (error) {
        ...
        try {
            await db.query(`UPDATE contest_submissions SET overall_status = 'System Error' WHERE id = $1`, [submissionId]);
        }
        catch (dbError) {
        ...
        fs.unlink(outputPath, (err) => { if (err)
            console.error(`Error deleting .out file for contest sub ${submissionId}:`, err); });
    }
}
module.exports = {
    processSubmission,
