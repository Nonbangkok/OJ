...
const moveProblemsToContest = async (contestId, problemIds) => {
    ...
    try {
        ...
        const contestResult = await client.query('SELECT * FROM contests WHERE id = $1', [contestId]);
        ...
        const problemsCheck = await client.query('SELECT id, title, contest_id FROM problems WHERE id = ANY($1)', [problemIds]);
        ...
        const updateResult = await client.query('UPDATE problems SET contest_id = $1, is_visible = FALSE WHERE id = ANY($2) RETURNING id, title', [contestId, problemIds]);
...
const migrateSubmissionsAfterContest = async (contestId) => {
    ...
    `, [contestId]);
        ...
        await client.query('DELETE FROM contest_submissions WHERE contest_id = $1', [contestId]);
        // Move problems back to main system
        await moveProblemsBackToMain(contestId);
        // Update contest status to finished
        await client.query('UPDATE contests SET status = $1 WHERE id = $2', ['finished', contestId]);
        await client.query('COMMIT');
        return {
            success: true,
...
const getProblemsInContest = async (contestId) => {
    try {
        ...
        const contestResult = await db.query('SELECT status FROM contests WHERE id = $1', [contestId]);
        if (contestResult.rows.length === 0) {
            throw new Error('Contest not found');
        }
        const contest = contestResult.rows[0];
...
//# sourceMappingURL=problemMigration.js.map