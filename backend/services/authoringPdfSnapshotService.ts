...
import { taskCodeFromDraft } from '../authoring/pdfTemplate';
...
export async function capturePdfSnapshot(client: PoolClient, draft: ProblemDraftRow) {
  ...
  const snapshot = pdfSnapshotSchema.parse({
    document: { templateVersion: draft.template_version, title: draft.title, taskCode: taskCodeFromDraft(draft.problem_id, draft.title),
