...
export class AuthoringSpool {
  ...
  async storeInputs(id: string, directory: string): Promise<InputArtifact[]> {
    ...
    if (!names.length || names.length > TESTCASE_LIMITS.MAX_CASES) {
      throw new TestcaseError('invalid_case_count',
        `Generator must create 1–${TESTCASE_LIMITS.MAX_CASES} input files`);
    }
