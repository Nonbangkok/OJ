...
export async function compileJob(input: JobSnapshot, workRoot: string, options: { timeoutMs?: number; signal?: AbortSignal } = {}): Promise<JobResult> {
  ...
  try {
    ...
    const filename = job.kind === 'compile_generator' ? 'generator.cpp' : 'solution.cpp';
