...
export interface Profile { id: string; akaName: string; realName: string; defaultLanguage: string; countryCode: string }
...
export const draftBase = (id: string) => `/admin/authoring/drafts/${encodeURIComponent(id)}`;
export const isActive = (job: Job) => ['queued', 'compiling', 'running'].includes(job.status);
