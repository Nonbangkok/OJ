...
function ProfileSyncRuns({ onError }: { onError: (error: unknown) => void }) {
  ...
  return <section aria-label="Profile sync runs"><h3>Profile sync runs</h3>
...
export default function JobHistory({ jobs, onError }: { jobs: Job[]; onError: (error: unknown) => void }) {
  ...
  return <section><h2>Build history & logs</h2><p>Latest 100 jobs. Reloading this page preserves job history on the server.</p>
    ...
    {hasSyncJob && <p role="note">Profile sync rebuilds this draft's PDF with updated author metadata and republishes it if it was published. A failed sync leaves the previous PDF live.</p>}
