  ...
  return <fieldset className={styles.metadataFields} disabled={disabled}>
    ...
    <fieldset className={styles.categoryPicker}>
      ...
      <div className={styles.categoryOptions}>
        {PROBLEM_CATEGORIES.map(category => <label key={category} className={styles.categoryOption}>
          <input
            type="checkbox"
            checked={value.categories.includes(category)}
            onChange={e => onEdit('categories', e.target.checked
              ? [...value.categories, category].sort()
